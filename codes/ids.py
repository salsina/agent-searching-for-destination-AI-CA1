...
class Map:
    ...
    def is_in_explored(self,explored,state):
        for i in range(len(explored)):
            if state == explored[i]:
                return [i]
        return []
    
    def check_children_dls(self,curr_x,curr_y,node,explored,frontier,to_add,action,maxDepth):
        ...
        if child_state_tuple not in explored :
            ...
            explored[child_state_tuple] = child_state.depth
        else:
            if node.state.depth + 1 < explored[child_state_tuple] :
                distinct_states += 1
                explored[child_state_tuple] = child_state.depth
                child_node = Node(child_state,node,action)    
                frontier.append(child_node)
    ...
    def drop_the_ball(self,node,explored,frontier,curr_x,curr_y):
        ...
        if child_state_tuple not in explored :
            ...
            if node.state.depth < explored[child_state_tuple] :
                explored[child_state_tuple] = child_state.depth
                child_node = Node(child_state,node.parent ,node.action + ' & Drop')    
                frontier.append(child_node)
            return [explored, frontier]
    ...
    def pick_the_ball(self,node,explored,frontier,curr_x,curr_y):
        ...
        if child_state_tuple not in explored :
            ...
            if node.state.depth + 1 < explored[child_state_tuple] :
                explored[child_state_tuple] = child_state.depth
                child_node = Node(child_state,node,node.action + ' & Pick')    
                frontier.append(child_node)
                distinct_states += 1
                return [explored,frontier]
    ...
    def DLS(self,maxDepth):
        ...
        explored = {(self.init_state.coordinates,self.init_state.remaining_balls_starts,self.init_state.balls_to_drop):self.init_state.depth}
        ...
        while len(frontier) > 0:
            ...
            for to_add in DIRECTIONS:  
                if self.position_is_valid(to_add[:2],curr_x,curr_y):
                    found_states += 1
                    ans = self.check_children_dls(curr_x,curr_y,node,explored,frontier,to_add[:2],to_add[2],maxDepth-1)
                    if len(ans) == 2: 
                        explored, frontier = ans[0], ans[1]
                    else:
                        return [True,ans[0]]
