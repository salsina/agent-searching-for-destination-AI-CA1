...
class Map:
    def __init__(self,rows,cols,map,start_coordinates,end_coordinates,capacity,num_of_balls,balls_coordinates):
        ...
        init_heuristic = self.calculate_heuristic(start_coordinates,0,1)
        end_heuristic = self.calculate_heuristic(end_coordinates,0,1)
    ...
    def calculate_heuristic(self,coordinates,depth,alpha):
        return alpha*max(abs(self.end_coordinates[0] - coordinates[0]),abs(self.end_coordinates[1] - coordinates[1])) + depth
    ...
    def check_children(self,curr_x,curr_y,node,explored,to_add,action,alpha):
        global distinct_states 
        child_coordinates = [curr_x + to_add[0] , curr_y + to_add[1]]  
        heuristic = self.calculate_heuristic(child_coordinates,node.state.depth+1,alpha)          
        ...
        while len(self.frontier) > 0:
            ...
            for to_add in DIRECTIONS:  
                if self.position_is_valid(to_add[:2],curr_x,curr_y):
                    found_states += 1
                    ans = self.check_children(curr_x,curr_y,node,explored,to_add[:2],to_add[2],alpha)
                    if len(ans) == 1:
                        return ans[0]
                    else:
                        explored = ans[0]
...
print('------------------------------------------------')
print('alpha = 6')
found_states, distinct_states = 1,1
m.ans = []
t1 = time()
x = m.A_star(6)
t2 = time()
